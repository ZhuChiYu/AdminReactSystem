...
const MyTasks = () => {
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      message.error('获取周任务列表失败');
      console.error('获取周任务列表失败:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    },
    {
      ...
      render: (status: number) => {
        const statusMap = {
          ...
          [FollowUpStatus.IN_PROGRESS]: { color: 'processing', text: '进行中' },
          [FollowUpStatus.COMPLETED]: { color: 'success', text: '已完成' },
          [FollowUpStatus.PAUSED]: { color: 'warning', text: '已暂停' }
        };
  ...
  const StatisticsCards = () => {
    ...
    const { summary, total } = getStatistics(tasks, TaskType.CONSULT);
